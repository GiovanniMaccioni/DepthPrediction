...
def train_batch_depth_estimation(model, train_loader, criterion, optimizer, epoch, device):
    ...
    for i, images in enumerate(train_loader):
        ...
        images = torch.cat((images, images, images), dim = 1)#FIXME For resnet and densenet
        ...
        loss = criterion(out, images[:,0,:][:,None,:])#Select one channel for supervision(for resnet)
        loss.backward()
        optimizer.step()
...
def train_batch_depth_prediction(model, train_loader, criterion, optimizer, epoch, device):
    ...
    for i, images in enumerate(train_loader):
        ...
        #images = images.squeeze()#FIXME added to unify channels and sequence length; to sub it for reshape
        #images.shape: [batch_size, sequence_length, channels, height, width]
        #FIXME here I reshape the tensor to noit have a sequence dimension, and collapse it with the channels.
        #images = images.reshape((images.shape[0], images.shape[1]*images.shape[2], images.shape[3], images.shape[4] ))#----> NOT NEEDED FOR 3D CONVOLUTIONS
        optimizer.zero_grad()
        out, latent = model(images[:, 0, :])
        loss = criterion(out, images[:, 1, :])#Selected the last frame of the sequence to reconstruct it. 0 to select the first frame(depth reconstruction)
        loss.backward()
        optimizer.step()
        ...
        if i == 0:
            ...
            min_ = images[0, 0, :].detach().cpu().min().float()
            image = torchvision.transforms.functional.to_pil_image(images[0, 0, :], mode=None)
            previous = wandb.Image(image, caption=f"Previous, range {min_:0.3f}, {max_:0.3f}")

            max_ = images[0, 1, :].detach().cpu().max().float()
            min_ = images[0, 1, :].detach().cpu().min().float()
            image = torchvision.transforms.functional.to_pil_image(images[0, 1, :], mode=None)
            gt = wandb.Image(image, caption=f"Ground truth, range {min_:0.3f}, {max_:0.3f}")
            ...
            imgs = [gt, predicted, previous]#, mean_f]
...
def evaluate_batch(model, loader, criterion, device):
    ...
    with torch.no_grad():
        for i, images in enumerate(loader):
            ...
            loss = criterion(out, images[:,0,:][:,None,:])#images[:, 0, :][:, None])
...
def evaluate_batch_prediction(model, loader, criterion, device):
    model.eval()
    accuracy = 0
    running_loss = 0
    num_batches = len(loader)
    ...
    with torch.no_grad():
        for i, images in enumerate(loader):
            ...
            if i == 0:
                ...
                max_ = images[0, 1, :].detach().cpu().max().float()
                min_ = images[0, 1, :].detach().cpu().min().float()
                image_gt = torchvision.transforms.functional.to_pil_image(images[0, 1, :], mode=None)
                gt = wandb.Image(image_gt, caption=f"Ground truth, range {min_:0.3f}, {max_:0.3f}")
