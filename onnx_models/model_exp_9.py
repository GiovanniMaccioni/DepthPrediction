...
class Encoder(nn.Module):
    def __init__(self):
        ...
        self.conv7 = ConvBlockRes(28, 32, 3)
        self.conv8 = ConvBlockRes(32, 36, 3)
...
class Autoencoder_conv(nn.Module):
    def __init__(self, encoder, decoder):
        ...
        self.linear4 = nn.Linear(4096, 46656)
    ...
    def forward(self, input_sequence):
        ...
        x = self.linear1(x)
        x = self.actv(x)
        latent_vector = self.linear2(x)

        x = self.linear3(latent_vector) 
        x = self.actv(x)
        x = self.linear4(x)
        #x = self.unflatten(x)
